...
public:
  JaccardRet(uint64_t n)
      : num_nodes(n),
        upper_triangle((double*)mmap(
            NULL, sizeof(double) * n * (n - 1) / 2, PROT_READ | PROT_WRITE,
            MAP_PRIVATE | MAP_ANONYMOUS | MAP_POPULATE, -1, 0)) {
    assert(upper_triangle != MAP_FAILED);
  }

  ~JaccardRet() {
    munmap(upper_triangle, sizeof(double) * num_nodes * (num_nodes - 1) / 2);
  }

  void reset() {
    std::fill(upper_triangle, upper_triangle + num_nodes * (num_nodes - 1) / 2,
              0);
  }
...
public:
  ...
  double get_val_unsafe(uint64_t n1, uint64_t n2) { return blah; }
...
struct Jaccard_Algo {
  ...
  template <typename IntersectAlgorithm, typename JRet>
  void JaccardImplSinglePair(Graph& graph, uint64_t node0, uint64_t node1,
                             JRet& ret) {
    uint64_t sz = graph.size();
  ...
  template <typename IntersectAlgorithm, typename JRet>
  void JaccardImplPair(Graph& graph, size_t node_samp, JRet& ret,
  ...
  void JaccardImpl(Graph& graph, JRet& ret) {
    ...
    galois::do_all(
        galois::iterate(idx_it(0), idx_it(size * (size - 1) / 2)),
        [&](const uint64_t linear_index) {
          ...
          const uint64_t i =
              size - 2 -
              uint64_t(
                  floor(sqrt(-8.0L * k + 4.0L * n * (n - 1.0L) - 7.0L) / 2.0L -
                        0.5L));
          const uint64_t j = linear_index + i + 1 - size * (size - 1) / 2 +
                             (size - i) * ((size - i) - 1) / 2;
  ...
  void JaccardImplRand(Graph& graph, JRet& ret, std::vector<uint64_t>& samps) {
    galois::do_all(galois::iterate((size_t)0, samps.size()),
                   [&](size_t start_node) {
                     this->JaccardImplPair<IntersectAlgorithm>(
                         graph, start_node, ret, samps);
                   });
  }
};

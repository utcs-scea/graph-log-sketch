...
void RMAT(const RMAT_args_t& args, uint64_t ndx,
  ...
  for (uint64_t i = 0; i < args.scale; i++) {
    ...
    if (rand <= args.A) {
      src = (src << 1) + 0;
      dst = (dst << 1) + 0;
    } else if (rand <= args.A + args.B) {
      src = (src << 1) + 0;
      dst = (dst << 1) + 1;
    } else if (rand <= args.A + args.B + args.C) {
      src = (src << 1) + 1;
      dst = (dst << 1) + 0;
    } else if (rand <= args.A + args.B + args.C + args.D) {
      src = (src << 1) + 1;
      dst = (dst << 1) + 1;
    }
  }
  ...
  if (src != dst) { // do not include self edges
    if (src > dst)
      std::swap(src, dst); // make src less than dst
...
int main(int argc, char** argv) {
  ...
  if (argc != 9) {
    if (myrank == 0)
      printf("Usage: <nthreads> <seed> <scale> <edge ratio> <A> <B> <C> "
             "<outfile-tag>\n");
    return 1;
  }
  ...
  uint64_t num_edges =
      (myrank < num_big_ranks) ? tot_edges / nprocs + 1 : tot_edges / nprocs;
  uint64_t num_bumps = num_big_ranks * (tot_edges / nprocs + 1) +
                       (myrank - num_big_ranks) * (tot_edges / nprocs);
  ...
  std::pair<uint64_t, uint64_t>* edges =
      new std::pair<uint64_t, uint64_t>[num_edges];
  ...
  galois::do_all(
      ...
      [&](uint64_t i) { RMAT(rmat_args, i + num_bumps, edges); },
      galois::steal());
  galois::runtime::getHostBarrier().wait();
  t02 = MPI_Wtime();

  if (myrank == 0)
    std::cerr << "Graph creation time:"
              << "\t" << t02 - t01 << std::endl;

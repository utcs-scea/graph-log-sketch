...
void sssp_bfs(uint64_t src, T* g) {
  ...
  while (!nextSt->empty()) {
    ...
    loop(galois::iterate(*currSt), [&](const uint64_t& curr) {
  ...
  SECTION( "Trivial Queries" )
  {
    auto f = [c]<bool par, bool ehp>(std::string s)
    {
      ...
      run_test_and_benchmark<RetVal*>(s,c,3,
        ...
        {
          uint64_t end;
          RetVal* r = (RetVal*) malloc(sizeof(RetVal)); r->g = new Graph<par,
  ehp>(); r->g->ingestNodes(100, end); return r;
        },
        [](RetVal* r){ for(uint8_t i = 0; i < 100; i++) r->n[i] =
  shortest_length_dijkstras(i,i,r->g); },
    ...
    auto f = [c]<bool par, bool ehp>(std::string s)
    {
      ...
      run_test_and_benchmark<RetVal*>(s,c,3,
        []
        {
          uint64_t end;
          RetVal* r = (RetVal*) malloc(sizeof(RetVal)); r->g = new Graph<par,
  ehp>(); r->g->ingestNodes(2, end); end = 1; r->g->ingestEdges(1, 0, &end);
          return r;
        },
    ...
    auto f = [c]<bool par, bool ehp>(std::string s)
    {
      ...
      run_test_and_benchmark<RetVal*>(s,c,3,
        []
        {
          uint64_t end;
          RetVal* r = (RetVal*) malloc(sizeof(RetVal)); r->g = new Graph<par,
  ehp>(); r->g->ingestNodes(101, end); for(uint64_t i = 1; i < 101; i++)
            r->g->ingestEdges(1, i-1, &i);
          return r;
        },
        [](RetVal* r){ for(uint8_t i = 0; i < 100; i++) r->n[i] =
  shortest_length_dijkstras(i,i+1,r->g); },
      ...
      run_test_and_benchmark<uint64_t>(
          ...
          [g](uint64_t v) {
            for (uint64_t i = 0; i < g->get_num_nodes(); i++) {
              for (uint64_t ni = 0; ni < g->get_num_nodes(); ni++)
                g->get_node(ni)->val = UINT64_MAX;
              sssp_bfs(i, g);
            }
          },
    ...
    auto f = [c]<bool par, bool ehp>(std::string s)
    {
      ...
      run_test_and_benchmark<uint64_t>(s,c,6,
        ...
        [g](uint64_t v)
        {
          ...
          {
            for(uint64_t ni = 0; ni < g->get_num_nodes(); ni++)
  g->get_node(ni)->val = UINT64_MAX; sssp_bfs(i,g);
          }
        },
        [](uint64_t v){},
        [](uint64_t v){});

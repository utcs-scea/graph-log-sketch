...
TEST_CASE("Running Simple Dijkstras", "[bfs]") {
  ...
  SECTION("Trivial Graph") {
    auto f = [c]<bool par, bool ehp>(std::string s) {
      ...
      run_test_and_benchmark<RetVal*>(
          s, c, 3,
          [] {
            RetVal* r = (RetVal*)malloc(sizeof(RetVal));
            r->g      = new Graph<par, ehp>();
            r->g->ingestNode();
            return r;
          },
      ...
      run_test_and_benchmark<RetVal*>(
          ...
          [](RetVal* r) {
            for (uint8_t i = 0; i < 100; i++)
              r->n[i] = shortest_length_dijkstras(i, i, r->g);
          },
          [](RetVal* r) {
            for (uint8_t i = 0; i < 100; i++)
              REQUIRE(r->n[i] == 0);
  ...
  SECTION("Many One Edge Queries") {
    auto f = [c]<bool par, bool ehp>(std::string s) {
      ...
      run_test_and_benchmark<RetVal*>(
          ...
          [](RetVal* r) {
            for (uint8_t i = 0; i < 100; i++)
              r->n[i] = shortest_length_dijkstras(i, i + 1, r->g);
          },
          [](RetVal* r) {
            for (uint8_t i = 0; i < 100; i++)
              REQUIRE(r->n[i] == 1);
  ...
  SECTION("All Pairs of Nodes Citeseer") {
    auto f = [c]<bool par, bool ehp>(std::string s) {
      ...
      run_test_and_benchmark<RetVal*>(
          ...
          [](RetVal* r) {
            for (uint64_t i = 0; i < r->g->get_num_nodes(); i++)

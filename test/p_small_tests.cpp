...
using LS_CSR = galois::graphs::LS_LC_CSR_64_Graph<
    void, void>::with_out_of_line_lockable<true>::type;

void test_edge_list(uint64_t num_nodes,
  ...
  if (repeats) {
    sort_edge_list =
        new std::priority_queue<uint64_t, std::vector<uint64_t>,
                                std::greater<uint64_t>>[num_nodes]();
    for (auto p : vec) {
    ...
    refer = new LC_CSR(
        num_nodes, vec.size(),
        [sort_edge_list](uint64_t n) { return sort_edge_list[n].size(); },
        [sort_edge_list](uint64_t n, uint64_t e) {
          auto v = sort_edge_list[n].top();
          sort_edge_list[n].pop();
          return v;
        },
  ...
  for (uint64_t i = 0; i < repeats; i++) {
    in_quest = new LS_CSR(
        num_nodes, 0, [](uint64_t v) { return 0; },
        [](uint64_t v, uint64_t e) { return 0; },
        [](uint64_t v, uint64_t e) { return 0; });
    ...
    galois::for_each(
        ...
        galois::loopname("for_each_add_edges"), galois::no_pushes());

    galois::do_all(
        galois::iterate(*in_quest),
        [&](const uint64_t j) {
          ...
          while (in_quest_edge_iter != in_quest_edge_end &&
                 refer_edge_iter != refer_edge_end) {
            REQUIRE(in_quest->getEdgeDst(*in_quest_edge_iter) ==
                    refer->getEdgeDst(*refer_edge_iter));
            in_quest_edge_iter++;
            refer_edge_iter++;
          }
...
void test_edge_list_p_check(uint64_t num_nodes,
                            std::vector<std::pair<uint64_t, uint64_t>> vec,
                            uint64_t repeats = 0) {
  LC_CSR* refer;
  LS_CSR* in_quest;
...
void test_p_construct(uint64_t num_nodes,
                      std::vector<std::pair<uint64_t, uint64_t>> vec,
                      uint64_t repeats = 10) {
  LC_CSR* refer;
  LS_CSR* in_quest;
  ...
  if (repeats) {
    ...
    for (auto p : vec) {
      edge_list[p.first].emplace_back(p.second);
    }

    galois::do_all(
        galois::iterate((uint64_t)0, num_nodes),
        [&](uint64_t n) {
          std::sort(edge_list[n].begin(), edge_list[n].end());
        },
        galois::steal());
...
void test_edge_list(Func f, uint64_t num_nodes,
                    std::vector<std::pair<uint64_t, uint64_t>> vec,
                    uint64_t repeats = 0, uint64_t counts = 8) {
  LC_CSR* refer;
  LS_CSR* in_quest;
...
TEST_CASE("Parallel Edits Yield the Same Graph", "[parallel]") {
  ...
  SECTION("Add Two Edges") { test_edge_list_p_check(2, {{0, 1}, {1, 0}}); }
...
TEST_CASE("Parallel Edits Using Different Algorithms", "[parallel]") {
  ...
  SECTION("Add Four Edges") {
    test_edge_list(add_edges_per_node<LS_CSR>, 2,
                   {{0, 0}, {0, 1}, {1, 0}, {1, 1}});
    test_edge_list(add_edges_group_insert_sort<LS_CSR>, 2,
                   {{0, 0}, {0, 1}, {1, 0}, {1, 1}});
  }
  SECTION("Add Citeseer") {
    uint64_t num_nodes;
...
TEST_CASE("Parallel Construction Yields the Same Graph", "[parallel]") {
  ...
  SECTION("Add Two Edges") { test_p_construct(2, {{0, 1}, {1, 0}}); }
  SECTION("Add Four Edges") {

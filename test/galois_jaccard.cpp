...
#include "katana/jaccard.h"

#include "katana/AnalyticsUtils.h"
...
katana::Result<void> katana::Jaccard(const std::shared_ptr<PropertyGraph>& pg,
  ...
  case katana::JaccardPlan::kUnknown:
...
katana::JaccardAssertValid(const std::shared_ptr<PropertyGraph>& pg,
  ...
  if (abs(graph.GetData<katana::JaccardSimilarity>(GNode{compare_node}) - 1.0) >
      EPSILON) {
    return katana::ErrorCode::AssertionFailed;
  }

  auto is_bad = [&graph](const GNode& n) {
    auto& similarity = graph.template GetData<katana::JaccardSimilarity>(n);
...
katana::JaccardStatistics::Compute(const std::shared_ptr<PropertyGraph>& pg,
  ...
  katana::do_all(
      ...
      [&](const GNode& i) {
        double similarity = graph.GetData<katana::JaccardSimilarity>(i);
  ...
  return katana::JaccardStatistics{

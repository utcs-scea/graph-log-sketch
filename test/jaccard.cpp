...
struct Jaccard_Algo {
  ...
  JaccardStatistics compute(Graph& graph, GNode compare_node) {
    ...
    return JaccardStatistics{max_similarity.reduce(), min_similarity.reduce(),
  ...
  public:
    ...
    const uint64_t operator()(GNode n2) {
      ...
      while (edges_n2_iter != edges_n2_end &&
             edges_base_iter != edges_base_end) {
  ...
  void run_jaccard_all_nodes(const std::string& fn, pa c,
  ...
  template <typename T>
  void run_jaccard_all_nodes_inc(
      uint64_t num_nodes, uint64_t num_edges,
      const std::vector<std::pair<uint64_t, uint64_t>>& ret, pa c,
      const std::string& msg, T add) {
    auto g = [num_nodes]() {
      return new Graph(
...
galois::graphs::MorphGraph<unsigned, void, true>::with_no_lockable<true>::type;
  ...
  {
    lclo.run_jaccard_all_nodes("../graphs/citeseer.el", c, "Citeseer
LS_CSR_LOCK_OUT"); lc6g.run_jaccard_all_nodes("../graphs/citeseer.el", c,
"Citeseer LC_CSR_64_GRAPH"); lc3g.run_jaccard_all_nodes("../graphs/citeseer.el",
c, "Citeseer LC_CSR_32_GRAPH");
    morg.run_jaccard_all_nodes("../graphs/citeseer.el", c, "Citeseer
MORPH_GRAPH");
  }
  ...
  {
    lclo.run_jaccard_all_nodes("../graphs/flickr.el", c, "Flickr
LS_CSR_LOCK_OUT"); lc6g.run_jaccard_all_nodes("../graphs/flickr.el", c, "Flickr
LC_CSR_64_GRAPH"); lc3g.run_jaccard_all_nodes("../graphs/flickr.el", c, "Flickr
LC_CSR_32_GRAPH"); morg.run_jaccard_all_nodes("../graphs/flickr.el", c, "Flickr
MORPH_GRAPH");
  }
...
TEST_CASE("Galois Edits Jaccard", "[jaccard]") {
  ...
  SECTION("All Nodes Citeseer Galois") {
    ...
    lclo.run_jaccard_all_nodes_inc(num_nodes, num_edges, ret, c,
                                   bench + " LC_CSR_LOCK_OUT", lclo_add);
    lc6g.run_jaccard_all_nodes_inc(num_nodes, num_edges, ret, c,
                                   bench + " LC_CSR_64_GRAPH", lc6g_add);
    lc3g.run_jaccard_all_nodes_inc(num_nodes, num_edges, ret, c,
                                   bench + " LC_CSR_32_GRAPH", lc3g_add);
    morg.run_jaccard_all_nodes_inc(num_nodes, num_edges, ret, c,

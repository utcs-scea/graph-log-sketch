...
void gen_1_hop_features(Graph* graph, GNode vertex) {
  ...
  for (auto it = graph->edge_begin(vertex); it != graph->edge_end(vertex);
       it++) {
...
void gen_2_hop_features(Graph* graph) {
  ...
  galois::do_all(
      ...
      [&](auto& vertex) { gen_1_hop_features(graph, vertex); },
      galois::steal());
...
torch::Tensor export_edge_list_to_torch(
...
// Note this is a bad implementation do not use it unless you explicitly have
// to.
template <typename Graph, typename VertexType, typename EdgeType>
std::tuple<std::pair<std::vector<int64_t>, std::vector<int64_t>>,
           std::unordered_map<uint64_t, VertexType>>
_build_ego_graph_serial(Graph& g, uint64_t start, uint64_t end,
  ...
  while (level < levels.size()) {
    ...
    for (auto currEL = startEL; startEL != endEL; currEL++) {
      ...
      if (vertex_set.find(glbID) == vertex_set.end()) {
        if (max_neighbors <= added_neighbors)
          continue;
...
template <uint64_t N, typename Key, size_t (*hash)(Key), typename Val,
...
struct bucketed_unordered_map {
  ...
  bool insert_exclusive_reference(Key k, Val& v, Val** ret) {
    return insert_exclusive_reference<use_lock>(
        k, v, ret, []() {}, []() {});
  }
};
...
std::tuple<torch::Tensor,
           bucketed_unordered_map<256, uint64_t, u64_u8_hash, VertexType>>
_build_ego_graph_parallel(const Graph& g, uint64_t start, uint64_t end,
                          std::vector<uint64_t> levels = {5, 3, 2, 1, 0}) {
  std::atomic<uint64_t> localID = 0;
  std::atomic<uint64_t> edgeID  = 0;

  galois::InsertBag<std::pair<uint64_t, const VertexType&>> curr_front,
      next_front;
  ...
  // setup initials
  galois::do_all(galois::iterate(start, end), [&](uint64_t root) {
    VertexType V;
    bool inserted;
    inserted = vertex_set.insert_exclusive_reference(g.getData(root), &V);
    if (!inserted)
      exit(-2); // TODO proper error logging
    ...
    galois::do_all(
        ...
        [&](std::pair<uint64_t, const VertexType&> src) {
          ...
          for (auto currEL = startEL; startEL != endEL; currEL++) {
            ...
            auto t = [&enough_space, &added_neighbors, &max_neighbors,
                      &localID](VertexType* U) {
              if (added_neighbors.fetch_add(1, std::memory_order_relaxed) <
                  max_neighbors) {
                enough_space = true;
                U->lid       = localID.fetch_add(1, std::memory_order_relaxed);
              }
            };
            bool inserted = vertex_set.insert_exclusive_reference(
                glbID, g.getData(glbID), &U, t, []() {});
...
_build_ego_graph(const Graph& g, uint64_t start, uint64_t end,
                 std::vector<uint64_t> levels = {5, 3, 2, 1, 0}) {
  auto tup = _build_ego_graph_serial<Graph, VertexType, EdgeType>(g, start, end,
                                                                  levels);
  return std::tuple(export_edge_list_to_torch(std::get<0>(tup)),
                    std::get<1>(tup));
}

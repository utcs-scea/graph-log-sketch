 ...
 */
#ifndef GRAPH_LOG_SKETCH_BETWEENNESSCENTRALITY_MRBC_BITSET_HH_
#define GRAPH_LOG_SKETCH_BETWEENNESSCENTRALITY_MRBC_BITSET_HH_
...
class MRBCBitSet : public galois::DynamicBitSet {
  ...
  // Member functions
  inline size_t get_word(size_t pos) const {
    return pos < bits_uint64 ? 0 : pos / bits_uint64;
  }
  inline size_t get_offset(size_t pos) const {
    return pos < bits_uint64 ? pos : pos % bits_uint64;
  }
  inline uint64_t get_mask(size_t pos) const {
    return uint64_t(1) << get_offset(pos);
  }

#if defined(REVERSE_MODE) || defined(FLIP_MODE)
  size_t reverse(size_t pos) { return pos == npos ? npos : num_bits - pos - 1; }
#endif
  ...
  void flip_recursive(size_t pos) {
    ...
    if (!(bitvec[rBlock] & rMask)) {
      ...
      while (!bitvec[rBlock].compare_exchange_weak(r_old, r_old | rMask,
...
// using Base::size;
  ...
  MRBCBitSet(const MRBCBitSet& other) {
    resize(other.size());
    auto& bv = other.get_vec();
    std::copy(bv.begin(), bv.end(), this->bitvec.begin());
#ifdef USE_INDICATOR
    indicator = other.indicator;
#endif
  }
  MRBCBitSet& operator=(const MRBCBitSet& other) {
    resize(other.size());
    auto& bv = other.get_vec();
    std::copy(bv.begin(), bv.end(), this->bitvec.begin());
#ifdef USE_INDICATOR
    ...
    if (ret != val) {
      ...
      if (val) {
        while (!bitvec[get_word(pos)].compare_exchange_weak(
            old_val, (old_val | get_mask(pos)), std::memory_order_relaxed)) {
        }
      } else {
        while (!bitvec[get_word(pos)].compare_exchange_weak(
            old_val, (old_val & ~get_mask(pos)), std::memory_order_relaxed)) {
  ...
  void set(size_t index) {
    ...
    if ((bitvec[bit_index] & bit_mask) == 0) { // test and set
      ...
      while (!bitvec[bit_index].compare_exchange_weak(
          old_val, old_val | bit_mask, std::memory_order_relaxed)) {
      }
    }
  }
#endif
...
// using Base::resize;
   ...
   */
  bool nposInd() { return indicator == npos; }
#endif
  /**
   * Returns: the lowest index i such as bit i is set, or npos if *this has no
   * on bits.
   */
  size_t find_first() const { return find_from_block(0); }

  size_t find_last() const { return find_from_block(bitvec.size() - 1, false); }
  ...
  /**
   * Returns: the lowest index i greater than pos such as bit i is set, or npos
  ...
  size_t find_prev(size_t pos) const {
    ...
    return res ? curBlock * bits_uint64 + left_most_bit(res)
   ...
   */
  size_t forward_indicator() { return forward_iterate(indicator); }

  /**
   * To move indicator to the next set bit.
   */
  size_t backward_indicator() { return backward_iterate(indicator); }
#endif
};
#endif // GRAPH_LOG_SKETCH_BETWEENNESSCENTRALITY_MRBC_BITSET_HH_

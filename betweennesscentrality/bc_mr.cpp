...
constexpr static const int numRuns = 1;
constexpr static const bool output = false;
...
static unsigned int
    numSourcesPerRound(1); // "sourcesToUse: Sources to use in BC"
static std::string
    sourcesToUse(""); // "numRoundSources: Number of sources to use for APSP"
static uint64_t
    totalNumSources(0); // "numOfSources: Total number of sources to do BC"
static bool useSingleSource(false); // "singleSource: Use a single source."
static uint64_t startNode(0);       // "startNode: Single source start node."
static unsigned int vectorSize(
    0); // "vectorSize: DEBUG: Specify size of vector used for node data"
...
using Graph = galois::graphs::DistLocalGraph<NodeData, void>;
...
void BackPropOp(GNode dst, Graph& graph) {
  ...
  if (i != infinity) {
    ...
    float toAdd = (1.0 + dst_data.sourceData[i].dependencyValue) /
                  dst_data.sourceData[i].shortPathCount;
...
void BackProp(Graph& graph, const uint32_t lastRoundNumber) {
  ...
  while (currentRound <= lastRoundNumber) {
    ...
    // since we're using the transpose graph
...
int main(int argc, char** argv) {
  ...
  if (argc < 3) {
    std::cerr << "Usage: " << argv[0]
              << " <filename> <numVertices> <num_sources>\n";
    return 1;
  }
  uint64_t numVertices = std::stoul(argv[2]);
  totalNumSources      = std::stoul(argv[3]);
  unsigned int rseed   = argc > 4 ? std::stoul(argv[4]) : 0;
  galois::DistMemSys G;
  ...
  if (sourceVector.empty()) {
    sourceVector.resize(hg->size());
    std::iota(sourceVector.begin(), sourceVector.end(), 0);

    if (totalNumSources > 0 && totalNumSources != hg->size()) {
      std::mt19937 gen(rseed);
      std::shuffle(sourceVector.begin(), sourceVector.end(), gen);
      sourceVector.resize(totalNumSources);
    }
  }
  ...
  for (auto run = 0; run < numRuns; ++run) {
    ...
    while (offset < nodeBoundary && totalSourcesFound < totalNumSources) {
      ...
      // galois::gDebug("Current BC scores:");
      // for (auto&& curNode: hg->allNodesRange()) {
      //   NodeData& cur_data = hg->getData(curNode);
      //   galois::gDebug(curNode, " ", cur_data.bc);
      // }

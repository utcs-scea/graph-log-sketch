...
struct EdgeEntry {
  ...
  inline bool atomic_is_tomb() {
    if (parallel)
      return dest.atomic_is_tomb();
    else
      return dest.is_tomb();
  }

  inline void set_dest(uint64_t d) { dest.set_value(d); }
};
...
struct NodeEntry {
  ...
  uint64_t stop;
  struct empty {};
  [[no_unique_address]] std::conditional_t<std::is_same_v<NodeTy, void>, empty,
                                           NodeTy>
      val;
  // false mean that the value is TOMBSTONED
  inline bool lock() {
...
class Graph {
  ...
  uint64_t no_overflow_average(uint64_t n, uint64_t m) const {
...
public:
  Graph()
      : num_nodes(0), edge_end(0),
        nodes((
            NodeEntry<parallel, NodeTy>*)(ehp ? mmap(NULL, 1 << 30,
                                                     ...
                                                     -1, 0)
                                              : malloc(1 << 30))),
        edges((EdgeEntry<parallel>*)(ehp ? mmap(NULL, 1 << 30,
                                                PROT_READ | PROT_WRITE,
                                                MAP_PRIVATE | MAP_ANON |
                                                    MAP_HUGETLB |
                                                    (30 << MAP_HUGE_SHIFT),
  ...
  inline EdgeEntry<parallel>* get_edge_unsafe(uint64_t e) {
    return this->edges + e;
  }

  inline NodeEntry<parallel, NodeTy>* get_node(uint64_t n) {
    if (num_nodes.get_value() > n)
      return nodes + n;
    if (parallel) {
      num_nodes.lock();
  ...
  inline NodeEntry<parallel, NodeTy>* get_node_unsafe(uint64_t n) {
    return this->nodes + n;
  }
...
private:
  void edge_placement(uint64_t start, uint64_t stop, uint64_t num_orig,
                      uint64_t num_new, uint64_t* dest, uint64_t place_start,
                      uint64_t place_stop) {
    ...
    while (edges_left) {
      ...
      while (orig_curr < stop && get_edge(orig_curr)->is_tomb())
        orig_curr++;

      uint64_t orig_curr_dest =
          (orig_curr < stop) ? get_edge(orig_curr)->get_dest() : UINT64_MAX;
      uint64_t dest_curr_dest =
          (dest_curr < num_new) ? dest[dest_curr] : UINT64_MAX;
      uint64_t swap_curr_dest =
          (swap_stop - swap_start) ? swap_arr[swap_start] : UINT64_MAX;
      ...
      if (orig_curr < stop && place_curr == orig_curr &&
          (orig_curr_dest > dest_curr_dest ||
           orig_curr_dest > swap_curr_dest)) {
        if (!swap_arr)
          swap_arr = (uint64_t*)malloc(sizeof(uint64_t) * num_orig);
      ...
      // This takes care of the equal case of dest_curr_dest == swap_curr_dest
        ...
        dest_curr++;
        if (dest_curr_dest == swap_curr_dest) {
          swap_start++;
          edges_left--;
        }
  ...
  EdgeEntry<parallel>* binarySearchAcquire(uint64_t find, uint64_t start,
                                           uint64_t stop,
                                           EdgeEntry<parallel>* e_list) {
    if (start == stop)
      return nullptr;
...
public:
  ...
  void ingestEdges(uint64_t num_new_edges, uint64_t src, uint64_t* dst) {
    ...
    if (ns->lock()) {
      ...
      if (num_orig + num_new_edges <= diff) {
        ...
        uint64_t new_start = (stop == this->edge_end.get_value())
                                 ? start
                                 : this->edge_end.get_value();
        ...
        edge_placement(start, stop, num_orig, num_new_edges, dst, new_start,
                       new_stop);
  ...
  uint64_t ingestEdgeList(uint64_t num_nodes, const std::vector<uint64_t>& srcs,
  ...
  uint64_t
  ingestEdgeList(uint64_t num_nodes,
  ...
  convertible_to<std::ranges::range_value_t<T>, std::pair<uint64_t, uint64_t>>

...
std::vector<std::pair<uint64_t,uint64_t>> gen_random_graph_edges(uint64_t num_nodes, uint64_t num_props, double prob_prop, uint64_t& num_edges)
{
  ...
  galois::do_all(galois::iterate((uint64_t)0, num_nodes),
    [&](const uint64_t src){
      ...
      for(uint64_t p = 0; p < num_props; p++)
      {
        ...
        if(prob_prop > val) bag.emplace(src, p);
      }
    });
...
std::vector<uint64_t>* gen_random_to_edge_list(uint64_t num_nodes, uint64_t num_props, double prob_prop, uint64_t& num_edges)
{
    ...
    Setup setup, Algo algo)
{
  ...
  run_benchmark<uint64_t>(statsfn, msg, c,
    [&graph,setup]{setup(graph); return 0;},
    [&graph,algo](uint64_t v){algo(graph);},
    [](uint64_t v){});
}
...
template<typename Graph>
void add_edges_one_by_one(Graph* graph, uint64_t lower, uint64_t upper, uint64_t batch_size, const std::vector<std::pair<uint64_t, uint64_t>>& edges, uint64_t count)
{
  for(uint64_t start = lower; start < upper; start += batch_size)
  {
    galois::for_each(galois::iterate(edges.begin() + lower, edges.begin() + std::min(lower + batch_size, edges.size())),
...
template<typename Graph>
void add_edges_count_insert_sort(Graph* graph, uint64_t lower, uint64_t upper, uint64_t batch_size, const std::vector<std::pair<uint64_t, uint64_t>>& edges, uint64_t count)
{
  for(uint64_t start = lower; start < upper; start += batch_size)
  {
    ...
    //std::memset(edge_count, 0, graph->size() * sizeof(std::atomic<uint64_t>));
    T_mem_alloc.stop();
    ...
    galois::do_all(galois::iterate(graph->begin(), graph->end()),
        [&](const uint64_t src)
        {
          ...
          if(sz) graph->insertEdgesSerially(src, sz, lower, edges);
        }, galois::loopname(("Insertion " + std::to_string(count)).c_str()),
        galois::steal());
...
/*
template<typename Graph>
void add_edges_weave_insert_sort(Graph* graph, uint64_t lower, uint64_t upper, uint64_t batch_size, std::vector<std::pair<uint64_t, uint64_t>>& edges, uint64_t count)
{
  for(uint64_t start = lower; start < upper; start += batch_size)
  {
    ...
    galois::StatTimer T_mem_freed(("Memory Free " + std::to_string(count)).c_str());
    T_mem_alloc.start();
    edge_count = new galois::substrate::PerThreadStorage<CountFirstLast>[graph->size()]();
    T_mem_alloc.stop();

    galois::do_all(galois::iterate(lower, std::min(lower + batch_size, edges.size())),

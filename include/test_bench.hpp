...
void check_el_file_and_benchmark(pa c, std::string ELFile) {
  std::string cmd = "head -n 1 " + ELFile + " && tail -n +2 " + ELFile +
                    " | sort -t' ' -k1,1n -k2,2n";
  ...
  auto f = [c, &edges, numNodes, &s_edges,
            s_num_nodes]<bool par, bool ehp>(std::string s) {
    run_test_and_benchmark<Graph<par, ehp>*>(
        s, c, 7, [] { return new Graph<par, ehp>(); },
        [&edges, numNodes](Graph<par, ehp>* g) {
          g->ingestEdgeList(numNodes, edges);
        },
  ...
  auto g = [c, &edges, numNodes, &s_edges, s_num_nodes, ELFile]() {
    ...
    run_test_and_benchmark<RType>(
        "Sequentially Ingest Graph from " + ELFile + "Galois", c, 7,
        [] {
          RType r{new MGraph(), new vector<GNode>()};
          return r;
        },

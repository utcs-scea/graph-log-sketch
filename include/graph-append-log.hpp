  ...
  PackedPointer
{
  ...
  bool invalidate()
  {
    ...
    if (t >> 63) return false;
    return __atomic_compare_exchange_8(&point, &t, 1<<63 | t, true, __ATOMIC_RELAXED, ATOMIC_RELAXED);
  }
  bool change(T* n, T&* o, int memory_model = __ATOMIC_RELAXED)
  {
    uint64_t o = __atomic_load_8(&point, __ATOMIC_RELAXED);
    if (o >> 63) return false;
    return __atomic_compare_exchange_8(&point, &o, 0xEFFF & (uint64_t) n, true, memory_model, memory_model);
  }
  ...
  bool isInvalid()
  {
    return (__atomic_load_8(&point, __ATOMIC_RELAXED) >> 63);
  }
  bool isAlloc()
  {
    return !(__atomic_load_8(&point, __ATOMIC_RELAXED));
  }
...
struct Graph
{
  ...
  //Traverses tombstoned items in order to generate a local version of the Node.
  NodeEntry<NodeTy*, EdgeTy> acquire_node_read(uint64_t node_id)
  {
    //This part might be racy only matters when aggressive transitions are enabled
    auto vo = vertex_orig;
    auto vl = vertex_log;
    ...
    do
    {
      curr = next;
      node = TOMB;
      while(node == TOMB) next = curr->dode.getPointer(node);
    } while(node == DEAD)
    return NodeEntry {next, curr->node.stop.getPointer(node), 1, &curr->node.t};
  }
  ...
  void ingestNewEdge(uint64_t src, uint64_t dest, EdgeTy e)
  {
    ...
    //Construct new NodeEntry<NodeTy, EdgeTy> pointing to deep copy of edge objects

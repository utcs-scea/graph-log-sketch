...
public:
  ...
  static void compute(scea::graph::MutableGraph& g) {
    ...
    galois::do_all(
      ...
      galois::no_stats(), galois::loopname("ComputeOutDegrees"), galois::steal()
    );

    for (int iter = 0; iter < MAX_ITERATIONS; ++iter) {
      std::for_each(newRank.begin(), newRank.end(), [](std::atomic<double>& n) { n.store(0.0); });
      ...
      galois::do_all(
        ...
        galois::no_stats(), galois::loopname("DistributeContributions"), galois::steal()
      );

      galois::do_all(
        galois::iterate(0ul, numNodes),
        [&](uint64_t i) {
          newRank[i].store((1.0 - DAMPING_FACTOR) / numNodes + DAMPING_FACTOR * newRank[i].load(std::memory_order_relaxed), std::memory_order_relaxed);
        },
        galois::no_stats(), galois::loopname("ComputeNewRanks"), galois::steal()
      );
      ...
      galois::do_all(
        galois::iterate(0ul, numNodes),
        [&](uint64_t i) {
          diff += std::abs(newRank[i].load(std::memory_order_relaxed) - rank[i]);
          rank[i] = newRank[i].load(std::memory_order_relaxed);
        },
        galois::no_stats(), galois::loopname("CheckConvergenceAndUpdate"), galois::steal()
      );

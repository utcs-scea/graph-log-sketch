...
cols = ["Edit", "Algo"]
rows = [12.5, 25, 37.5, 50, 62.75, 75, 87.5, 100]
...
for x in range(0,3):
  data.append(np.zeros((8,2)))
  for i in range(0,2):
    for j in range(0,8):
      print(x)
      print(data)
      data[x].itemset((j,i), float(lines[x* 2 * 8 + j * 2 + i]))
...
# Then, just call :
plot_clustered_stacked([csr, lsr, mor],["CSR", "LS_CSR", "MorphGraph"])

...
echo EXEC:" $EXEC " INPUT:" $INPUT " SET:"$SET " QUEUE:"$QUEUE " PART:"$PART
...
for task in $SET; do
	...
	echo "CPU-only " $EXEC $INPUT $PART $1 $threads ${EXEC}_${INPUT}_${PART}_${1}
	# sbatch run_ls6.sbatch $EXEC $INPUT $PART $1 $threads ${GRAPH_TYPE}
	source run_ls6.sbatch $EXEC $INPUT $PART $1 $threads
	#rm run_ls6.sbatch
done
